...
- (NSString *)decodeBarcodeFromImage:(NSImage *)image {
	...
	// Create dmtx image.
	DmtxImage *dmtxImage = dmtxImageCreate((unsigned char*)[imageData bytes], image.size.width, image.size.height, DmtxPack32bppRGBX);
	if(dmtxImage == NULL)
    {
    ...
    DmtxTime dmtxTimeout = dmtxTimeAdd(dmtxTimeNow(), 2000);
	...
	for(count = 0; count < 1; count++) {
		...
		dmtxRegion = dmtxRegionFindNext(dmtxDecode, &dmtxTimeout);
        if(dmtxRegion == NULL) {
			break;
        }
